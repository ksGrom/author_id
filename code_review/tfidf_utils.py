...
def one_work_as_test(
        df: pd.DataFrame,
        work_title: str
) -> tuple[pd.DataFrame, pd.DataFrame, pd.Series, pd.Series]:
    """Функция для разбиения датасета на тренировочный и тестовый так,
    чтобы в тестовом было одно выбранное (`work_title`) произведение.
    """
...
def leave_one_title_out_cv_predictions(clf, df, n_jobs=6):
    """Функция обучает N моделей (где N - число произведений в датасете `df`).
    Для i-го произведения модель обучается на отрывках остальных N-1
    произведений, после чего применяется на отрывках i-ого произведения;
    полученные предсказания добавляются в вектор `predictions`.
    Функция возвращает вектор предсказаний `predictions` и обученные
    классификаторы в списке `classifiers`.
    """
    dataset_list = []
    for work_title in df.work_title.unique():
        X_train, X_test, y_train, y_test = one_work_as_test(df, work_title)
...
def leave_one_title_out_cv_score(
    ...
    """Функция для "leave-one-title-out"-кроссвалидации.
    Принимает на вход либо предсказания `predictions`, полученные
    в результате работы функции `leave_one_title_out_cv_predictions`,
    либо классификатор `clf` (тогда предсказания вычисляются с помощью
    упомянутой функции).
    Возвращает `predictions`, список произведений (в том порядке,
    в котором они обрабатывались), список значений f1-оценки,
    средняя f1-оценка.
    """
